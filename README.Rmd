...
Once the model is fitted, we can estimate the hidden components of any intraday signal based on all its observations, which is called **smoothing**. By calling `smooth_volume_model` function, we obtain daily, seasonal, and intraday dynamic components. This procedure helps us better identify the underlying information of the intraday signal.
...
To see how well our model performs on new data, we use `forecast_volume_model` function to do one-bin-ahead forecast on the out-of-sample dataset of 20 days.
