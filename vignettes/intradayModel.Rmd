...
modelSpec_fit <- uniModelFit(data_train, modelSpec, 
                             acceleration = TRUE, verbose = 0)
```
...
plot_decomposition(data_train, components)
```

With the fitted model, we can do one-bin-ahead prediction and evaluate its accuracy.

```{r}
predict_result <- uniModelPred(data, modelSpec_fit, out.sample = 20)
