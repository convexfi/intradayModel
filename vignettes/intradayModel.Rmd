...
unimodel_fit <- fit_unimodel(fdx_volume_fit, verbose = 2, control = list(acceleration = TRUE))
```
...
```{r}
smooth_result <- smooth_unimodel(fdx_volume_fit, unimodel_fit)
str(smooth_result)
```
...
```{r}
forecast_result <- forecast_unimodel(fdx_volume, unimodel_fit, out.sample = 20)
str(forecast_result)
```
