...
Next, we fit a univariate state-space model using `fit_volume` function. To be specific, we use the first 104 trading days for fitting, and the last 20 days for evaluation of forecasting performance.
...
```{r}
smooth_result <- use_model(purpose = "analysis", model_fit, aapl_volume_training)
...
```{r}
forecast_result <- use_model(purpose = "forecast", model_fit, aapl_volume, burn_in_days = 104)
...
> **forecast_unimodel**(data, unimodel, out_sample) 

`forecast_unimodel` function allows you to forecast the intraday signal one-bin-ahead, given a dataset and a fitted model. The one-bin-ahead prediction is mathematically denoted by $\hat{y}_{\tau+1} = \mathbb{E}[y_{\tau+1}|\{y_{j}\}_{j=1}^{\tau}]$. You need to indicate how many days from the end of the dataset to keep for out-of-sample prediction.

Since fitting a model to some data does not guarantee good prediction on unseen data, `forecast_unimodel` helps to evaluate the model's performance with the following measures:
...
```{r}
forecast_result <- forecast_unimodel(fdx_volume, model_fit, out_sample = 20)
str(forecast_result)
```
