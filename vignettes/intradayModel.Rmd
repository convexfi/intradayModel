...
plots$original_and_forecast # plot forecast result
```
...
> **decompose_volume**(purpose = "analysis", model, data)

`decompose_volume` function with `purpose = analysis` and a fitted model allows you to analyze every dataset based on all its observations. It applies the technique called Kalman smoothing.

This step decomposes the intraday trading signal into its daily, seasonal, and intraday dynamic components. The daily component and intraday dynamic component at time $\tau$ are the smoothed state estimate conditioned on all the data, and denoted by $\mathbb{E}[\mathbf{x}_{\tau}|\{y_{\tau}\}_{\tau=1}^{N}]$, where $N$ is the total number of bins in the dataset. Besides, the seasonal component has the value of $\boldsymbol{\phi}$.
...
Here $M$ is the total number of bins.
...
> **decompose_volume**(purpose = "forecast", model, data, burn_in_days = 0)

`decompose_volume` function with `purpose = forecast` and a fitted model allows you to forecast the intraday signal one-bin-ahead on a new dataset. The one-bin-ahead prediction is mathematically denoted by $\hat{y}_{\tau+1} = \mathbb{E}[y_{\tau+1}|\{y_{j}\}_{j=1}^{\tau}]$. 
...
forecast_result <- decompose_volume(purpose = "forecast", model_fit, fdx_volume, burn_in_days = 105) 
str(forecast_result)
```

Function `generate_plot` visualize the one-bin-ahead forecast components and the forecasting performance.
