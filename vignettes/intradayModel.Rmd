...
> **use_model**(purpose = "analysis", model, data)

`use_model` function with `purpose = analysis` and a fitted model allows you to analyze every dataset based on all its observations. It applies the technique called Kalman smoothing.
...
```{r}
analysis_result <- use_model(purpose = "analysis", model_fit, fdx_volume_training)
str(analysis_result)
```
...
> **use_model**(purpose = "forecast", model, data, burn_in_days = 0)

`use_model` function with `purpose = forecast` and a fitted model allows you to forecast the intraday signal one-bin-ahead on a new dataset. The one-bin-ahead prediction is mathematically denoted by $\hat{y}_{\tau+1} = \mathbb{E}[y_{\tau+1}|\{y_{j}\}_{j=1}^{\tau}]$. 
...
forecast_result <- use_model(purpose = "forecast", model_fit, fdx_volume, burn_in_days = 105) 
str(forecast_result)
```
