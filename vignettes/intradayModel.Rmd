...
Next, we fit a univariate state-space model using the `uniModelFit` function. To be specific,  we use the first 104 trading days for fitting, and the last 20 days for evaluation of forecasting performance.

```{r}
data <- AAPL_volume
data_train <- AAPL_volume[, 1:104]
model_fitted <- uniModelFit(data_train, fixed.pars = NULL, init.pars = NULL, verbose = 1, control = list(acceleration = TRUE))
```

Once the model is fitted, we can estimate the hidden components of the intraday signal based on all observations, which is called **smoothing**. By calling the `uniModelSmooth` function, we obtain daily, seasonal, and intraday dynamic components. This procedure helps us better identify the underlying information of the intraday signal.

```{r}
smooth_result <- uniModelSmooth(data_train, model_fitted)
str(smooth_result)
plot_components(smooth_result) # plot smoothed hidden components
plot_performance(smooth_result) # plot fitting result
```

To see how well our model performs on new data, we use the `uniModelForecast` function to do one-bin-ahead prediction on the out-of-sample dataset of 20 days.

```{r}
forecast_result <- uniModelForecast(data, model_fitted, out.sample = 20)
str(forecast_result)
...
The observed intraday signal can be written in the multiplicative combination of the components [@brownlees2011intra]:
...
To use this package, you have two choices of data types:

- a 2D numeric matrix of size `(n_bin, n_day)`;

- an xts object.

To help you get started, we provide two sample datasets: a matrix-type `AAPL_volume` and an xts-type `FDX_volume_xts`. 
...
> **uniModelFit**(data, fixed.pars  = NULL, init.pars = NULL, verbose = 0, control = list(acceleration = FALSE, maxit = 3000, abstol = 1e-4, log.switch = TRUE))

To fit a univariate state-space model, you can use the `uniModelFit` function. If you want to fix some parameters to specific values, you can provide a list of values to `fixed.pars`. If you have prior knowledge of the initial values for the unfitted parameters, you can provide it through `init.pars`. `verbose` controls the level of print. Further control options can be set via `control`. 
...
```{r, warning=FALSE}
data <- FDX_volume_xts
data_train <- FDX_volume_xts['2019-07-01/2019-11-30']
model_fitted <- uniModelFit(data_train, verbose = 2, control = list(acceleration = TRUE))
```
...
Since fitting a model to some data does not guarantee good prediction on unseen data, `uniModelPred` also evaluates the model's performance with the following measures for you:
